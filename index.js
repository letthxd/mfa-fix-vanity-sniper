...
function readMfaToken(force = false) {
  const now = Date.now();
  try {
    const stats = fs.statSync('mfa_token.json');
    if (mfaToken && stats.mtimeMs <= lastMfaFileTime && !force) return mfaToken;
    lastMfaFileTime = stats.mtimeMs;
    const data = fs.readFileSync('mfa_token.json', 'utf8').trim();
    const parsed = JSON.parse(data);

    if (parsed && parsed.token) {
      if (parsed.token !== mfaToken) {
        mfaToken = parsed.token;
        console.log(`mfa gecildi dosti: ${mfaToken}`);
      } else {
        mfaToken = parsed.token;
      }
